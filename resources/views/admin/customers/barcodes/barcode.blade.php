    ...
    <section class="content">
        ...
        <div class="box box-primary">
            ...
            <div class="box-header with-border">
                ...
                <form action="{{ URL::to('admin/customers/barcode')}}" method="get">
            ...
            <div class="box-body row" id="print-area">
                ...
                @if($barcodes->count() > 0)
                        ...
                        @foreach($barcodes as $index=>$barcode)
                            @if(!empty($barcode->barcode) && $barcode->barcode != null)
                                <div class="col-xs-12 text-center" id="printThisBarcode" style="cursor:pointer;" >
                                    <?php
                                        if(!empty($barcode->barcode) && $barcode->barcode != null){
                                            echo '<img style="padding-top: 15px"  src="data:image/png;base64,' . base64_encode($generator->getBarcode($barcode->barcode ?? 'null', $generator::TYPE_CODE_128)) . '"> ';
                                        }
                                        ?>
                                        <span style="display: block;" class="text-center">
                                        {{-- Nada ---  --}}
                                        {{$barcode->first_name . ' '.$barcode->last_name }} ---
