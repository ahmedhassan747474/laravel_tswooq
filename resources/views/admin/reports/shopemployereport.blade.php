...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1>{{ trans('labels.Employee') }} <small>{{ trans('labels.Employee') }}...</small> </h1>
    <ol class="breadcrumb">
      <li><a href="{{ URL::to('admin/dashboard/this_month')}}"><i class="fa fa-dashboard"></i> {{ trans('labels.breadcrumb_dashboard') }}</a></li>
      <li class="active">{{ trans('labels.Employee') }}</li>
    </ol>
  </section>
        ...
        <div class="box">
          <div class="box-header">
                <div class="col-lg-9 form-inline" id="contact-form">
                    ...
                    <form method="get" action="{{url('admin/shopemployereport')}}">
                        ...
                        <div class="col-xs-2">
                          <div class="form-group">
                            ...
                            <select class="form-control admin" name="admin_id">
                        ...
                        <div class="col-xs-2">
                          <div class="form-group">
                            {{-- <label for="exampleInputEmail2">{{ trans('labels.Shop') }}</label> --}}
                            <select class="form-control employe" name="employe_id">
                              <option value="">{{ trans('labels.SelectEmploye') }}</option>
...
<script>
  ...
  $(document).on('change', '.admin', function(){
    ...
    $.ajax({
      url: "{{url('admin/getEmployee')}}",
      type: "GET",
      data: {'admin_id' : admin_id},
      dataType: 'json',
      success: function (data) {
        var html_select_option = '<option value="">{{ trans("labels.SelectEmploye") }}</option>';
        for (index = 0; index < data.length; ++index) {
          html_select_option += '<option value="' + data[index].id + '">' + data[index].name + '</option>';
        }
        $('.employe').html(html_select_option);
      }
    });
  });
</script>
