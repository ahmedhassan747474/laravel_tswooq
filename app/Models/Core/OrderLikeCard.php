...
class OrderLikeCard extends Model
{
    ...
    public function paginator(){
        ...
        $language_id = '1';
        $data = DB::table('order_like_card')
            ->join('order_like_card_product', 'order_like_card_product.order_like_card_id', '=', 'order_like_card.order_like_card_id')
            ->orderBy('order_like_card.created_at', 'DESC');
            // ->where('customers_id', '!=', '');

        if(auth()->user()->role_id == 11) {
            $data->where('order_like_card.admin_id', '=', auth()->user()->id);
        } elseif(auth()->user()->role_id == 12) {
            $data->where('order_like_card.admin_id', '=', auth()->user()->parent_admin_id);
        }
        ...
        foreach ($orders as $orders_data) {
            $orders_products = DB::table('order_like_card_product')->sum('final_price');
