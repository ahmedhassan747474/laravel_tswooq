...
class Expenses extends Model
{
    ...
    public function expense_info(){
        return $this->belongsTo('App\Expense_info');
    }
    ...
    public function insert($request){
      ...
      //$date = strtotime($request->dob);
        ...
        $expenses_id = DB::table('users')->insertGetId([
            'role_id' => 2,
            'first_name' => $request->expenses_firstname,
            'last_name' => $request->expenses_lastname,
            'dob' => $request->expenses_dob,
            'gender' => $request->expenses_gender,
            'email' => $request->email,
            'country_code' => $request->country_code,
            'phone' => $request->expenses_telephone,
    ...
    public function addexpenseaddress($request){
    ...
    public function addressBook($address_book_id){
        $expense_addresses = DB::table('address_book')
            ->leftJoin('zones', 'zones.zone_id', '=', 'address_book.entry_zone_id')
            ->leftJoin('countries', 'countries.countries_id', '=', 'address_book.entry_country_id')
    ...
    public function zones($expense_addresses){

        $zones = DB::table('zones')->where('zone_country_id','=', $expense_addresses[0]->entry_country_id)->get();

      return $zones;
    }

    public function checkdefualtaddress($address_id){
        $expenses = DB::table('user_to_address')->where('address_book_id','=', $address_id)->get();
        return $expenses;
    }
    ...
    public function updateaddress($request){
        ...
        $expense_addresses =  $this->addresses($request->user_id);
        return 'success';
    }
    ...
    public function edit($id){
      ...
      $expenses = DB::table('users')->where('id','=', $id)->first();
      return $expenses;
    }

    public function updaterecord($expense_data,$user_id,$user_data){
      DB::table('users')->where('id', '=', $user_id)->update($user_data);
      DB::table('expenses')->where('user_id', '=', $user_id)->update($expense_data);
