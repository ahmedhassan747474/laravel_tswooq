  ...
  //addnewadmin
	public function addnewadmin(Request $request){
		...
		if(count($existEmail)>0){
            ...
            if ($request->shopImg){
                $image = $request->file('shopImg');
                $file_name = $image->getClientOriginalName();
                $request->shopImg->move(public_path('web/images/miscellaneous/shopImg/'), $file_name);
                $admin->shopImg = $file_name;
            }
            $admin->user_name =   $request->first_name.'_'.$request->last_name.time();
						$admin->first_name  = $request->first_name;
						$admin->last_name			 		=   $request->last_name;
						$admin->phone	 					=	$request->phone;
						$admin->email	 					=   $request->email;
						$admin->password		 			=  Hash::make($request->password);
						$admin->status		 	 		=   $request->isActive;
						$admin->avatar	 				=	 $uploadImage;
						$admin->record_number	 		    =   $request->record_number;
						$admin->role_id					=	 $request->adminType;
						$admin->parent_admin_id			=   $request->admin_id;
   ...
   public function profile(Request $request){
	...
	public function addrole(Request $request){
		...
		if(count($roles)>0){
			...
			$brands_view   	= $roles[0]->brands_view;
			$brands_create 	= $roles[0]->brands_create;
			$brands_update 	= $roles[0]->brands_update;
			$brands_delete 	= $roles[0]->brands_delete;
		...
		$result2[21]['permissions'] = array(
					'0'=>array('name'=>'brands_view','value'=>$brands_view),
					'1'=>array('name'=>'brands_create','value'=>$brands_create),
					'2'=>array('name'=>'brands_update','value'=>$brands_update),
					'3'=>array('name'=>'brands_delete','value'=>$brands_delete)
					);

		$result2[22]['link_name'] = 'Reviews';
		$result2[22]['permissions'] = array(
					'0'=>array('name'=>'reviews_view','value'=>$reviews_view),
					'1'=>array('name'=>'reviews_update','value'=>$reviews_update),
					);

		$result2[23]['link_name'] = 'Delivery Boy';
	...
	public function addnewroles(Request $request){
		...
		$roles = DB::table('manage_role')->where('user_types_id',$request->user_types_id)->insert([
						...
						'brands_view' => $request->brands_view,
						'brands_create' => $request->brands_create,
						'brands_update' => $request->brands_update,
						'brands_delete' => $request->brands_delete,
