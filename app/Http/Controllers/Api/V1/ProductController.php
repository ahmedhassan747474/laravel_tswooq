...
class ProductController extends BaseController
{
    ...
    public function get_all_group_products(Request $request){
        ...
        $responseData=[];
        if ($group_id){
            $products=Product::whereHas('groups',function($q) use($group_id){
            ...
            'products_ordered',
                ...
                $responseData = array('success' => '1', 'product_data' => $products, 'message' => "Returned all products.", 'total_record' => count($products));
            } else {
                $responseData = array('success' => '0', 'product_data' => [], 'message' => "Empty record.", 'total_record' => 0);
            }
        ...
        //     $categories->where('group_product.group_id',$group_id);
        ...
        if (!empty($request->filters)) {
            ...
            foreach ($request->filters as $filters_attribute) {
                ...
                $join->on('image_categories.image_id', '=', 'products.products_image')
                    ...
                    ->select('products.*','image_categories.path as products_image','products_attributes.*','products_options_values.*','products_options.*')
                // ->where('products_description.language_id','=', $language_id)
                // ->where('manufacturers_info.languages_id','=', $language_id)
                    ...
                    if(isset($request->search) ){
                        ...
                        $getProducts->where('products_options.products_options_name', 'LIKE', '%' . $searchValue . '%')->where('products_status', '=', 1);
                        
                        $getProducts->orWhere('products_options_values.products_options_values_name', 'LIKE', '%' . $searchValue . '%')->where('products_status', '=', 1);
                        
                        $getProducts->orWhere('products_name', 'LIKE', '%' . $searchValue . '%')->where('products.product_parent_id', '=', null)->where('products_status', '=', 1);

                    }
                    $getProducts =$getProducts->skip($skip)->take(10)
                    ->groupBy('products.products_id')
                    ->get();
                    // $getProducts->get();
                ...
                if (count($getProducts) > 0) {
                    foreach ($getProducts as $getProduct) {
                        if (!in_array($getProduct->products_id, $eliminateRecord)) {
                            ...
                            foreach ($products as $products_data) {
                                //check currency start
                                $products_data->products_image = asset(DB::table('image_categories')->where('image_id',$products_data->products_image)->first()->path);
                                // dd($products_data->products_image);
                                $requested_currency = $request->currency_code;
    ...
    //                 // if ($products_data->products_tock < $defaultStock && $products_data->products_tock > 0) {
    //                 //     $result[$index]->defaultStock = $products_data->products_tock;
    //                 // } else {
    //                 //     $result[$index]->defaultStock = $defaultStock;
    //                 // }
    ...
    public function getproductsbycategory(Request $request)
    {
        $products=Product::with('categories')->whereHas('categories',function($q) use($request){
            $q->where('categories.categories_id',$request->categories_id);
        })
        ->select(
        'products_id',
        'products_quantity',
        ...
        if (!empty($request->filters)) {
            ...
            foreach ($request->filters as $filters_attribute) {
                ...
                if (!empty($request->brand_id)) {
                    $data->where('products_to_categories.categories_id', '=', $request->brand_id);
                }
                    ...
                    foreach ($getProducts as $getProduct) {
                        if (!in_array($getProduct->products_id, $eliminateRecord)) {
                            ...
                            foreach ($products as $products_data) {
                                ...
                                // if ($products_data->products_tock < $defaultStock && $products_data->products_max_stock > 0) {
                                //     $result[$index]->defaultStock = $products_data->products_max_stock;
                                // } else {
                                //     $result[$index]->defaultStock = $defaultStock;
                                // }
            ...
            //get single category products
            if (!empty($request->brand_id)) {
                $categories->where('products_to_categories.categories_id', '=', $request->brand_id)
                    ->where('categories_description.language_id', '=', $language_id);
            }
    ...
    public function getproductbyid(Request $request){
        ...
        if (!empty($request->id)) {
            ...
            $categories->where('products.products_id',$request->id);
    ...
    public function getproductsyfavourite(Request $request){
        ...
        if($user)
        {
                ...
                ->select('products_to_categories.*', 'products.products_id','products.admin_id',
                ...
                'products.product_parent_id', 'products.products_slug',
    ...
    //         // ->where('products.products_id', '=', $product->product_id)
    //         // ->where('categories.parent_id', '!=', '0')
    //         ->where('products_to_categories.categories_id', $request->brand_id)
    //         ->where('product_parent_id', '=', null)
    //         ->paginate($request->limit);
